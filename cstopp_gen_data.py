...
def convert_line2label(line):
    elements = line.split('\n')[0].split(' ')
    num_class = int(elements[0])
    bbox = [float(element) for element in elements[1:5]]
    score = float(elements[5])
    y_d = float(elements[6])
    return num_class, bbox, score, y_d
...
def load_gt(f_truth):
    ...
    scores = []
    dists = []
    with open(f_truth,'r') as f_gt:
        for line in f_gt:
            labels = convert_line2label(line)
            classes.append(labels[0])
            boxes.append(labels[1])
            scores.append(labels[2])
            dists.append(labels[3])
    return np.array(classes),np.array(boxes),np.array(scores),np.array(dists)
...
def gen_data(split,list_dpath,out_path,fps_out,
    ...
    # Label generated by lidar
    opath_label_lidar = os.path.join(out_path,'label_lidar')
    # Label generated by cameras & gps
    ...
    if not os.path.exists(opath_label_lidar):
        os.makedirs(opath_label_lidar)
    ...
    for d_path in list_dpath:
        # Check whether there is a truth label file generated by using camera & GPS
        gt_path = os.path.join(d_path,'truth')
        ...
        for time_stamp in time_stamps:
            ...
            list_points = loadKrotations(input_lidar,start_time,
                                         n_frame,r_fps_lidar)
            # Save Lidar points as binary files
            ...
            for i_frame,image in enumerate(videogen):
                ...
                if i_frame % r_fps_cam == 0:
                    ...
                    out_label = os.path.join(opath_label_lidar,
                    ...
                    with open(out_label,'w') as f_label_lidar:
                    ...
                    # ---------------------- Ground Truth -----------------------
                    # Read and save Ground truth labels
                    # Generated by using camera & GPS
                    if is_gt:
                        in_gt = os.path.join(gt_path,
                                             _FILE_OUT.format(i_save)+'.txt')
                        out_gt = os.path.join(opath_label,
                                              _FILE_OUT.format(i_save)+'.txt')
                        
                        # Read generated ground truth labels
                        classes_gt,boxes_gt,scores_gt,dists_gt = load_gt(in_gt)

                        with open(out_gt,'w') as f_label:
                            # Format splitted by space.
                            # 1: class
                            # 4: bbox (ymin, xmin, ymax, xmax) (normalized 0~1)
                            # 1: score
                            # 1: distance (if dict_calib is not None)
                            for i_obj in range(boxes_gt.shape[0]):
                                if scores_gt[i_obj]>MIN_SCORE:
                                    line = [category_index[classes_gt[i_obj]]['name']]
                                    line += [str(coord) for coord in boxes_gt[i_obj]]
                                    line += [str(scores_gt[i_obj])]
                                    line += [str(dists_gt[i_obj])]
                                    f_label.write(' '.join(line)+'\n')
                        # Write video
                        if is_vout:
                            # Visualization of the truth label.
                            ...
                            vis_util.visualize_boxes_and_labels_on_image_array(
                                ...
                                line_thickness=2)
                            vwriter_gt.writeFrame(image_gt)
