...
def main(_):
    ...
    if os.path.exists(os.path.join(FLAGS.input,_FILE_VIDEO)):
        path_image = os.path.join(path_out,'image')
        gen_image = True
        if not os.path.exists(path_image):
            os.makedirs(path_image)
    if os.path.exists(os.path.join(FLAGS.input,_FILE_LIDAR)):
    ...
    # Load time stamps
    path_time = os.path.join(FLAGS.input,_FILE_TIMES)
    time_stamps = []
    ...
    if gen_image:
        ...
        input_video = os.path.join(FLAGS.input,_FILE_VIDEO)
                ...
                for time_stamp in time_stamps:
                    videogen = vreader(input_video,
                                       ...
                                       outputdict={'-r':str(FLAGS.fps_in),
                                                   '-ss':time_stamp[0],
                                                   '-t':time_stamp[1]})
                    print('<Image&Label> Time stamp start: {}, Duration: {} (secs)'.format(
                                    time_stamp[0],
                                    time_stamp[2]))
                    i_frame = 0
    ...
    if gen_lidar:
        ...
        input_lidar = os.path.join(FLAGS.input,_FILE_LIDAR)
        i_save = 0 # lidar name indexing

        for idx_t, time_stamp in enumerate(time_stamps):
            print('<LIDAR> Time stamp start: {}, Duration: {} (secs)'.format(
                                    time_stamp[0],
                                    time_stamp[2]))
            # Read LIDAR file
            ...
            for ldata in lidar_out:
                ldata[1].tofile(os.path.join(path_lidar,'{:06d}.bin'.format(i_save)))
                i_save += 1
